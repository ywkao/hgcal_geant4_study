...
#include "../SimTrackAna/interface/toolbox.h"
...
void hits::Init(TTree *tree)
{
    ...
    for(int i=0;i<28;i++) {
        ...
        h_rechit_amplitudeLow_layers[i] = new TH1D(hnamestr.str().c_str(), hnamestr.str().c_str(), 50, 0, 100000.);
    }
...
void hits::MakePlot(TString output)
{
    ...
    for(int i=0;i<28;i++) {
        c2->cd(i+1);
        h_rechit_amplitudeLow_layers [i] -> Draw();
    }
    ...
    for(int i=0;i<28;i++) {
        ...
        h_rechit_amplitudeLow_layers [i] -> Draw(); c1 -> SaveAs(hnamestr.str().c_str());
    }
}
void hits::MakeGraph(TString output, TString ytitle, TH1D** h)
{
    // logitudinal profile
    double x[28], y[28], ex[28], ey[28];
    for(int i=0;i<28;i++) {
        x[i]  = (double)(i+1);
        ex[i] = 0.;
        y[i]  = h[i]->GetMean();
    ...
    TGraphErrors *gr = new TGraphErrors(28, x, y, ex, ey);
    gr->SetTitle("");
    ...
    gr->Draw("acp");
    c3->SaveAs(output + ytitle + "_logitudinal_profile.png");
